...
function Header({ onReload, isLoading: isReloading }) {
  ...
  const handleUploadClick = () => {
    if (!isAuthenticated) {
      toast("Please login to upload videos");
      navigate("/login");
      return;
    }
    navigate("/upload");
  ...
  return (
    <header className="sticky inset-x-0 top-0 z-50 w-full border-b border-zinc-200 bg-white px-3 text-sm text-zinc-900 dark:border-zinc-800 dark:bg-[#0f0f0f] dark:text-white">
      ...
      <nav className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-2">
        ...
        {/* Right: create, notifications, profile, theme */}
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="hidden h-9 w-9 items-center justify-center rounded-full border border-zinc-200 bg-white text-zinc-800 hover:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100 dark:hover:bg-zinc-800 sm:inline-flex"
          ...
          <button
            ...
            className="hidden h-9 items-center justify-center rounded-full border border-zinc-200 bg-zinc-100 px-3 text-xs font-medium text-zinc-900 hover:bg-zinc-200 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-50 dark:hover:bg-zinc-800 md:inline-flex"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
            ...
            >
              <path d="M4 6.75A2.75 2.75 0 0 1 6.75 4h6.5A2.75 2.75 0 0 1 16 6.75V8h1.25A2.75 2.75 0 0 1 20 10.75v6.5A2.75 2.75 0 0 1 17.25 20h-6.5A2.75 2.75 0 0 1 8 17.25V16H6.75A2.75 2.75 0 0 1 4 13.25v-6.5Z" />
            </svg>
            Create
          </button>
          ...
          <button
            type="button"
            className="hidden h-9 w-9 items-center justify-center rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 md:inline-flex"
          ...
          <ReloadBtn onReload={onReload} isLoading={isReloading} />
        </div>

        {/* Mobile account / nav drawer trigger stays as-is */}
        <button className="group peer ml-1 flex w-7 shrink-0 flex-wrap gap-y-1.5 sm:hidden">
          <span className="block h-[2px] w-full bg-zinc-900 group-hover:bg-blue-600 dark:bg-blue-500 dark:bg-white" />
          <span className="block h-[2px] w-2/3 bg-zinc-900 group-hover:bg-blue-600 dark:bg-blue-500 dark:bg-white" />
          <span className="block h-[2px] w-full bg-zinc-900 group-hover:bg-blue-600 dark:bg-blue-500 dark:bg-white" />
        </button>
        <div className="fixed inset-y-0 right-0 flex w-full max-w-xs shrink-0 translate-x-full flex-col border-l border-white bg-[#121212] text-white duration-200 hover:translate-x-0 peer-focus:translate-x-0 sm:static sm:ml-4 sm:w-auto sm:translate-x-0 sm:border-none sm:bg-transparent sm:text-inherit">
          ...
          <ul className="my-4 flex w-full flex-wrap gap-2 px-4 sm:hidden">
            <li className="w-full">
              <Link to={'/'} className="flex w-full items-center justify-start gap-x-4 border border-white px-4 py-1.5 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:bg-zinc-100 dark:focus:bg-zinc-800">
            ...
            <li className="w-full">
              <Link to={'/mychannel/liked-videos'} className="flex w-full items-center justify-start gap-x-4 border border-white px-4 py-1.5 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:bg-zinc-100 dark:focus:bg-zinc-800">
                <span className="inline-block w-full max-w-[20px] group-hover:mr-4 lg:mr-4">
                  <svg
                    style={{ width: "100%" }}
                    viewBox="0 0 22 22"
            ...
            <li className="w-full">
              <Link to={'/mychannel/history-videos'} className="flex w-full items-center justify-start gap-x-4 border border-white px-4 py-1.5 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:bg-zinc-100 dark:focus:bg-zinc-800">
                <span className="inline-block w-full max-w-[20px] group-hover:mr-4 lg:mr-4">
                  <svg
                    style={{ width: "100%" }}
                    viewBox="0 0 22 20"
            ...
            <li className="w-full">
              <Link to={user ? `/mychannel/${user.username}` : '/login'} className="flex w-full items-center justify-start gap-x-4 border border-white px-4 py-1.5 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:bg-zinc-100 dark:focus:bg-zinc-800">
                <span className="inline-block w-full max-w-[20px] group-hover:mr-4 lg:mr-4">
                  <svg
                    style={{ width: "100%" }}
                    viewBox="0 0 22 16"
            ...
            <li className="w-full">
              <Link to={'/support'} className="flex w-full items-center justify-start gap-x-4 border border-white px-4 py-1.5 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:bg-zinc-100 dark:focus:bg-zinc-800">
                <span className="inline-block w-full max-w-[20px] group-hover:mr-4 lg:mr-4">
                  <svg
                    style={{ width: "100%" }}
                    viewBox="0 0 22 22"
            ...
            <li className="w-full">
              <Link to={'/mychannel/admin'} className="flex w-full items-center justify-start gap-x-4 border border-white px-4 py-1.5 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:bg-zinc-100 dark:focus:bg-zinc-800">
                <span className="inline-block w-full max-w-[20px] group-hover:mr-4 lg:mr-4">
                  <svg
                    style={{ width: "100%" }}
                    viewBox="0 0 22 22"
          ...
          <div className="mb-8 mt-auto flex w-full flex-wrap gap-4 px-4 sm:mb-0 sm:mt-0 sm:items-center sm:px-0">
            ...
            {!isAuthenticated ?
              <div className="flex items-center gap-2">
                <Link
                  to={"/login"}
                  className="flex items-center justify-center gap-2 rounded-[50px] border border-zinc-200 bg-white px-3 py-1.5 text-sm font-medium text-blue-600 hover:bg-blue-50 dark:border-zinc-700 dark:bg-transparent dark:text-blue-400 dark:hover:bg-zinc-800 sm:px-4"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
              ...
              <div className="flex w-full items-center gap-4 md:w-auto">
                <div className="relative" ref={profileRef}>
                  <button
                    ...
                    className="flex shrink-0 items-center justify-center rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800"
                  >
                    <img
                      src={user?.avatar}
                  ...
                  {isProfileOpen && (
                    <>
                      <div className="absolute right-0 z-20 mt-2 w-72 rounded-xl border border-zinc-200 bg-white p-3 shadow-xl dark:border-zinc-700 dark:bg-[#282828] dark:text-gray-100">
                        ...
                        <div className="mt-2 flex flex-col pt-1">
                          <button
                            type="button"
                            className="flex w-full items-center rounded-lg px-3 py-2 text-left hover:bg-zinc-100 dark:hover:bg-zinc-700"
                            onClick={() => {
                              navigate(`/mychannel/admin`);
                              setIsProfileOpen(false);
                            }}
                          >
                            YouTube Studio
